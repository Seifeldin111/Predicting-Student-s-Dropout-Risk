...
st.markdown("<h1 style='color: #00008B;'>Student Dropout Risk Assessment After Semester 2</h1>", unsafe_allow_html=True)
st.markdown("---")
...
with tab3:
      ...
      curr_units_2nd_sem_credited = st.number_input("Curricular units 2nd sem (credited)", min_value=0, step=1)
      curr_units_2nd_sem_enrolled = st.number_input("Curricular units 2nd sem (enrolled)", min_value=0, step=1)
      curr_units_2nd_sem_evaluations = st.number_input("Curricular units 2nd sem (evaluations)", min_value=0, step=1)
      curr_units_2nd_sem_approved = st.number_input("Curricular units 2nd sem (approved)", min_value=0, step=1)
      curr_units_2nd_sem_grade = st.number_input("Curricular units 2nd sem (grade)", min_value=0.0, step=0.1)
      curr_units_2nd_sem_without_eval = st.number_input("Curricular units 2nd sem (without evaluations)", min_value=0, step=1)
...
if all(v != "-- Select --" and v is not None for v in user_inputs.values()):
    if st.button("Submit"):
        ...
        pass_rate_sem2 = 0 if curr_units_2nd_sem_enrolled == 0 else curr_units_2nd_sem_approved / curr_units_2nd_sem_enrolled
        overall_pass_rate = (pass_rate_sem1 + pass_rate_sem2) / 2
        grade_improvement = curr_units_2nd_sem_grade - curr_units_1st_sem_grade


        def grade_imp_category(x):
            if x < 0:
                return "Negative"
            elif x == 0:
                return "No Change"
                ...
                return "Positive"
        grade_improvement = grade_imp_category(grade_improvement)

        consistency = 0 if (curr_units_1st_sem_enrolled + curr_units_2nd_sem_enrolled) == 0 else (curr_units_1st_sem_evaluations +  curr_units_2nd_sem_evaluations) / (curr_units_1st_sem_enrolled +  curr_units_2nd_sem_enrolled)

        total_enrolled = curr_units_1st_sem_enrolled + curr_units_2nd_sem_enrolled

        total_approved = curr_units_1st_sem_approved + curr_units_2nd_sem_approved

        drop_in_evaluations = curr_units_2nd_sem_evaluations - curr_units_1st_sem_evaluations

        eval_drop_ratio = 0 if curr_units_1st_sem_evaluations == 0 else (curr_units_2nd_sem_evaluations - curr_units_1st_sem_evaluations) / curr_units_1st_sem_evaluations

        fail_rate = 1 - overall_pass_rate
        ...
        if application_mode in ["1st phase—general contingent", "2nd phase—general contingent", "Over 23 years old",
                                "Change in course", "Technological specialization diploma holders",
        ...
        student_data = {
            ...
            "pass_rate_sem2": pass_rate_sem2,
            "overall_pass_rate": overall_pass_rate,
            "economic_stress": economic_stress,
            "grade_imp_category": grade_improvement,
            "consistency": consistency,
            "total_enrolled": total_enrolled,
            "total_approved": total_approved,
            "drop_in_evaluations": drop_in_evaluations,
        ...
        with open("pipeline10.skops", "rb") as f:
            untrusted = skops.io.get_untrusted_types(data=None, file=f)
            model = sio.load("pipeline10.skops", trusted=untrusted)

        with open("preprocessor10.skops", "rb") as f:
            untrusted = skops.io.get_untrusted_types(data=None, file=f)
            preprocessor = sio.load("preprocessor10.skops", trusted=untrusted)

        with open("X10_train.skops", "rb") as f:
            untrusted = skops.io.get_untrusted_types(data=None, file=f)
            shop_data = sio.load("X10_train.skops", trusted=untrusted)
        ...
        for i, feature in enumerate(values):
            values.iloc[i] = feature.replace("_", " ").title()
